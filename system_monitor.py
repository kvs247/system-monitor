...
class SystemMonitor:
    ...
    def _run(self) -> None:
        while self._running:
            ...
            print(
                f"{'CPU: ':{text_width}}{cpu_monitor.usage_total:.1f}%\n"

                f"{'RAM: ':{text_width}}{mem_monitor.mem_used_gib:.1f} GiB "
                f"({mem_monitor.mem_used_percent:.1f}%), "
                f"{mem_monitor.mem_free_gib:.1f} GiB free\n"
